...
const urlRegex = /^(https?:\/\/(www[0-9]?\.)?){1}(.*)$/

const chatSocketUrl = process.env.NODE_ENV === 'development' ? `http://${window.location.hostname}:${process.env.REACT_APP_SOCKET_PORT}` : `chat.${window.location.toString().match(urlRegex)!.pop()}`
